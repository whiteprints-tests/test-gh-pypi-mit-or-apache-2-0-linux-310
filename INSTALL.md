<!--
SPDX-FileCopyrightText: © 2026 Romain Brault <mail@romainbrault.com>

SPDX-License-Identifier: MIT OR Apache-2.0
-->

# ⚙️ Installation

Is is possible to install the [package] in the following ways.

## From PyPI

To install the [package] from [PyPI] run

```console
pip install test-gh-pypi-mit-or-apache-2-0-linux-310
```

[PyPI]: https://pypi.org/

## From source

To install with [pip] from [GitHub] run the command:

```console
pip install git+ssh://git@github.com/whiteprints-tests/test-gh-pypi-mit-or-apache-2-0-linux-310
```

[GitHub]: https://github.com
[git]: https://git-scm.com/

For additional information see [PyPA]'s guide: [installing Packages](https://packaging.python.org/en/latest/tutorials/installing-packages).

[PyPA]: https://www.pypa.io/en/latest/
[pip]: https://pip.pypa.io/en/stable
[package]: https://pypi.org/project/test-gh-pypi-mit-or-apache-2-0-linux-310
